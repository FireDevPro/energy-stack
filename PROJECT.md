...
- **Pre-registration is binding**. Once filed to OSF, hypotheses, arm definitions, arm calendar, metric definitions, statistical analysis plan, and decision rules lock at the OSF freeze tag (in-repo files cite the tag; literal commit SHA captured in OSF/Zenodo deposit metadata). Binding spec: [`docs/plans/sced-rebaseline-spec-2026-05-13.md`](docs/plans/sced-rebaseline-spec-2026-05-13.md).
...
- **Open data and open code at submission time**. Telemetry published as Apache Parquet on Zenodo (CC BY 4.0), with [Brick Schema](https://brickschema.org/) JSON-LD metadata. Repo will be tagged with the OSF freeze tag (target 2026-05-30, e.g. `osf-prereg-2026-05-30`); Zenodo will issue a citable code DOI from the tagged release. The literal commit SHA is captured in OSF/Zenodo deposit metadata; in-repo files cite the tag.
...
Practical implication for in-flight work: anything touching the scheduler or telemetry between now and OSF filing is on the critical path for the June 1 experiment start. Operational features that don't directly serve Arm A / Arm B / observability are parked. The thermal-model design line of work (`docs/archive/THERMAL_MODEL_DESIGN.md`) is parked as future-arm research and is NOT on the 2026 critical path — Arm B for this study is the existing scheduler per binding spec §3.
...
The collector layer matches "Current Architecture" above. Phase 3.4 dashboards, Phase 4 thermostat integration, Phase 8 bill ingest, Phase 9 PJM DM2 poller, and Phase 10 safety supervisor all landed by May 2026 (see Development Roadmap below). Active follow-on work: spec §11 pre-OSF deliverables for the SCED rebaseline (Arm B is the existing scheduler; no new controller variant needed). ComfortNet pipeline (broker + telegraf + Pi-3B publisher) is live as of 2026-05-06 and `hvac.comfortnet` is flowing — see [`docs/COMFORTNET_USE_CASES.md`](docs/COMFORTNET_USE_CASES.md) for downstream consumption and [`Promithius-DR/comfortnet`](https://github.com/Promithius-DR/comfortnet) for the live implementation.
...
SCED **deterministic-alternation** field study (rebaselined 2026-05-13) comparing the CTK04AE programmed schedule (Arm A) against the active hvac-scheduler (Arm B: RTP/DTOD/5CP-risk-aware RBC + safety supervisor), 14-day arm-level alternation, 2026-06-01 → 2026-11-16 (12 arms total, 6 Arm A + 6 Arm B). Binding spec: [`docs/plans/sced-rebaseline-spec-2026-05-13.md`](docs/plans/sced-rebaseline-spec-2026-05-13.md). Implementation plan: [`docs/plans/sced-rebaseline-implementation-2026-05-13.md`](docs/plans/sced-rebaseline-implementation-2026-05-13.md). Canonical arm calendar in [`tools/analysis/arm_calendar.py`](tools/analysis/arm_calendar.py) (analysis-side) and [`deploy/energy-stack/hvac-scheduler/arm_calendar.py`](deploy/energy-stack/hvac-scheduler/arm_calendar.py) (controller-side; byte-identical, CI hash-sync checked). Exclusion-rule mechanics: spec §5 + §5.1 (5-mode hour-level classification, single ≥259-fully-valid-hours validity gate, cost-matched symmetric exclusion within each pair). The original [`docs/archive/EXPERIMENT_DESIGN.md`](docs/archive/EXPERIMENT_DESIGN.md), `randomize_arms.py`, `experiment-assignments-summer-2026.csv`, the `20260601` seed, and the bootstrap CI / SCED-randomization-test framework are explicitly superseded for OSF-binding content (retained for historical context per spec §0). Appendix A controller constants extracted to [`docs/CONTROLLER_CONSTANTS.md`](docs/CONTROLLER_CONSTANTS.md); §O2 capacity reconstruction to [`docs/O2_CAPACITY_RECONSTRUCTION.md`](docs/O2_CAPACITY_RECONSTRUCTION.md); §11 ethics framing to [`docs/ETHICS_FRAMING.md`](docs/ETHICS_FRAMING.md). Capacity-risk operating window remains 2026-06-01 → 2026-09-30 per PJM Manual 19; arms 10-12 (October-November) extend the experiment for arm-vs-arm sample size beyond the 5CP-eligibility window.
...
- **Pre-OSF spec §11 deliverables**: critical path for the 2026-05-30 OSF filing and 2026-06-01 experiment start. The 13 binding-spec §11 items cover arm-calendar + mode-gating in hvac-scheduler (`SCHEDULER_MODE=experiment` reads locked [`arm_calendar.py`](tools/analysis/arm_calendar.py); writes Arm B periods only), mode telemetry (`hvac.arm_mode` every 5-min decision cycle), switch-event logging, input-feed health telemetry, controller heartbeat / watchdog, `rt_hrl_lmps` poller, DTOD analysis-rate table, CTK04AE Arm A schedule freeze ([`docs/THERMOSTAT_ARM_A_SCHEDULE.md`](docs/THERMOSTAT_ARM_A_SCHEDULE.md)), dry-run guard audit, analysis pipeline rewrite, NOAA fallback station selection, day-type schedule completeness verification, and the shadow validation run. Audit progress in [`docs/plans/pre-osf-doc-audit-execution-2026-05-18.md`](docs/plans/pre-osf-doc-audit-execution-2026-05-18.md).
- **OSF pre-registration filing**: binding spec [`docs/plans/sced-rebaseline-spec-2026-05-13.md`](docs/plans/sced-rebaseline-spec-2026-05-13.md) must be frozen (status: frozen + `frozen_at_tag: <tag name>`, e.g. `osf-prereg-2026-05-30`) before 2026-06-01. The literal commit SHA the tag points at is captured in OSF/Zenodo deposit metadata; in-repo files cite the tag (stable across squash/amend/rebase). Filing uses the OSF open-ended template with the spec attached via Zenodo DOI; mechanics in [`docs/OSF_FILING_MECHANICS.md`](docs/OSF_FILING_MECHANICS.md).
...
| `tools/analysis/arm_calendar.py` + `deploy/energy-stack/hvac-scheduler/arm_calendar.py` | Canonical deterministic 12-arm calendar (byte-identical, CI hash-sync checked). Replaces the retired `randomize_arms.py` + assignment CSV. |
| **Subsystem designs** | |
