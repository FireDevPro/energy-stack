...
4. **Outside-in TDD.** First step on any feature: write a feature-level acceptance test that exercises the full vertical slice end-to-end. Drive inward from there. Reason: prevents "task done equals feature done" confusion. The project is the whole, not any given task. Terminology: a **slice** is a phase (per plan-authoring rule below — each phase = vertical slice, demoable end-to-end). A **feature** is the whole multi-phase thing. **Intermediate stages** are the in-flight tasks *within* a phase. While intermediate stages are in flight the test stays xfail or skip; at the phase-shipping commit (PR) the test must produce a visible signal — that means **xfail (`strict=True`)**, not skip. Skip is silent across phase boundaries and trains the team to ignore the north star. The marker comes off the moment the test passes against the real implementation with zero scaffolding — that is the only definition of feature-complete.
