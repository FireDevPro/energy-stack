...
Committed at `.understand-anything/knowledge-graph.json` (1109 nodes, 1642 edges, 9 layers, 10-step tour). Built by the [Understand-Anything](https://github.com/Lum1104/Understand-Anything) plugin. Auto-update hook (`autoUpdate: true` in `.understand-anything/config.json`) re-patches incrementally on every commit; cosmetic-only commits stay token-free, structural changes patch only changed files. Reader's narrative onboarding: [docs/ONBOARDING.md](docs/ONBOARDING.md) (graph-derived, regenerate via `/understand-anything:understand-onboard` post-major-refactor).

For cross-file or flow questions ("what depends on X", "what touches Y", "where does this signal flow", "what's the blast radius of changing Z"), grep `.understand-anything/knowledge-graph.json` BEFORE grepping the live tree. The graph encodes import/call/tested_by/documents/configures/deploys/triggers edges that filesystem-grep misses in one pass. Node summaries also encode WHY a file exists, not just WHAT it imports.

Query pattern (per plugin's consumer-skill protocol; never dump the 1.1 MB file into context):

1. Grep for matching nodes by `"name"`, `"summary"`, or `"tags"` in the JSON.
2. Grep edges for those node IDs (as `"source"` or `"target"`) to get the 1-hop neighborhood.
3. Read only the relevant excerpts.

User-invokable skills (agents don't auto-call these; offer them when they fit):

- `/understand-anything:understand-chat <q>`: narrative answers from graph.
- `/understand-anything:understand-explain <path>`: deep-dive on a file or function.
- `/understand-anything:understand-diff`: blast-radius analysis before PR review; writes `diff-overlay.json` for the dashboard's red/yellow PR view.
- `/understand-anything:understand-dashboard`: visual exploration in browser.
- `/understand-anything:understand-onboard`: regenerate `docs/ONBOARDING.md`.

Graph is canonical for structural questions; INDEX.md is canonical for intent grouping. They are not redundant: graph is autogenerated and machine-queryable, INDEX.md is hand-curated and reflects editorial judgement on what matters.
