...
- **CI type gate (repo `typecheck.yml` runs on every push):** strict mypy over `hvac_scheduler` + import-linter. Consequences for every code task: (1) all new production code is FULLY type-annotated — every parameter and return; injected seams (`price_source`, `climate`, `telemetry`, `client`, `query_api`, `write_api`, `snap`, `own` where cross-module typing would couple pure modules) are annotated `Any` (`from typing import Any`), which strict mypy accepts; (2) each new `test_rev4_*` module is added to the **dedicated rev4 test-module `[[tool.mypy.overrides]]` block** in `pyproject.toml` (created during Task 1 execution — wider relaxation than the shared list: untyped/incomplete defs, any-generics, untyped-calls, unreachable; the plan's test fakes need it) **in the same commit that creates the module, with `pyproject.toml` in that commit's `git add` line**; (3) `controller/__main__.py` imports `influxdb_client` directly — add an `ignore_imports` entry `hvac_scheduler.controller.__main__ -> influxdb_client` to the import-linter contract in Task 11; (4) Task 11 ALSO removes the **temporary `hvac_scheduler.controller.*` `ignore_missing_imports` scaffolding block** (added at Task 1 so the acceptance test could import not-yet-existing modules; self-retiring per module, delete outright once all controller modules exist); (5) Task 15 removes the then-stale `hvac_scheduler.app` entries from `pyproject.toml` when `app.py` is deleted.
...
```bash
git add deploy/energy-stack/hvac_scheduler/test_rev4_acceptance.py pyproject.toml
...
```bash
git add deploy/energy-stack/hvac_scheduler/controller/ deploy/energy-stack/hvac_scheduler/test_rev4_config.py pyproject.toml
...
```bash
git add deploy/energy-stack/hvac_scheduler/controller/pricing.py deploy/energy-stack/hvac_scheduler/test_rev4_pricing.py pyproject.toml
...
```bash
git add deploy/energy-stack/hvac_scheduler/controller/ deploy/energy-stack/hvac_scheduler/test_rev4_loop.py pyproject.toml
...
```bash
git add deploy/energy-stack/hvac_scheduler/controller/tiers.py deploy/energy-stack/hvac_scheduler/test_rev4_tiers.py pyproject.toml
...
```bash
git add deploy/energy-stack/hvac_scheduler/controller/holds.py deploy/energy-stack/hvac_scheduler/test_rev4_holds.py pyproject.toml
...
```bash
git add deploy/energy-stack/hvac_scheduler/controller/ownhold.py deploy/energy-stack/hvac_scheduler/test_rev4_ownhold.py pyproject.toml
...
**Interfaces:**
- Produces: `@dataclass(frozen=True) ControlSnapshot(schedule_cool: float | None, cool_setpoint: float, heat_setpoint: float, hold_active: bool, hold_until_minutes: int | None, indoor_temp: float | None, humidity: float | None)`; `class TccClimateAdapter` with `async def snapshot()`, `async def push(cool, heat, until_minutes)`, `async def release()` — async end-to-end per the async-seam global constraint (NEVER `asyncio.run` inside; the loop awaits these), matching the async fakes: `snapshot()` = `get_climate()` refresh + getters (`get_schedule_cool_f`, `get_cool_setpoint_f`, `get_heat_setpoint_f`, `get_hold_mode() != "Off"` — Permanent counts as a hold, `get_hold_until_minutes`, `get_current_temperature_f`, `get_humidity`); `push` = `set_heat_setpoint_f(heat)` + `set_cool_setpoint_f(cool)` + `set_hold_until(time(hour=until//60, minute=until%60))`; `release` = `set_hold_mode("Schedule")`.
...
**Interfaces:**
- Produces: `check_hvac_action_errors` reworked: `PUSH_FAILURE_ALERT_N = 3` as a **module constant** (spec: "constant, seed 3" — not an env var); query the last N `hvac.actions` rows' `error` field (sorted desc, `|> limit(n: 3)` after `sort(columns: ["_time"], desc: true)`); alert only when **all N** are non-empty (excluding the two `hvac_mode_not_cooling` benign strings, preserved from the current filter). Key stays `hvac_error:<first-40-chars>`. This kills the single-transient double-bark observed 2026-07-05 while catching genuine consecutive failures (a missed spike engage).
...
def test_push_failure_alert_requires_three_consecutive():
    ...
    class Api:
        def __init__(self, rows): self._rows = rows
        def query(self, flux):
            return [_T([_R(v) for v in self._rows])] if self._rows else []
...
- [ ] **Step 1: Wire-up audit BEFORE deleting (memory: verify-production-wireup).** `git grep -nE "hvac_scheduler\.(app|price_overlay|controller_core|controller_config|decision_codes)"` and `git grep -n "from .app import\|from .price_overlay"` — every hit must be a file being deleted in this task, the knowledge graph, or docs. Also `git grep -n "hvac.overrides"`, `git grep -n "hvac.price_overlay"`, and `git grep -rn "decision_trace" deploy/energy-stack/cockpit/` — **cockpit is a second-class citizen by operator decree: it gets rebuilt AFTER rev 4 is running and conforms to the new telemetry; rev 4 does not conform to it.** Enumerate the expected cockpit breakage in the PR body (reason-coded, not fixed here); no compatibility work in this feature. Also clean `pyproject.toml`: remove the now-stale `hvac_scheduler.app` mypy overrides and import-linter `ignore_imports` entries in this same PR (a stale unmatched entry is a linter error). Also sweep `.env.example` and `docs/SERVICES.md` for now-dead knobs: `OVERRIDE_GRACE_MIN` (orphaned by Task 14) and `SCHEDULER_DECISION_TRACE_VERBOSE` (rev 4 traces always emit — retire it from compose env + docs).
...
```bash
git add -A deploy/energy-stack/ docs/SERVICES.md pyproject.toml
