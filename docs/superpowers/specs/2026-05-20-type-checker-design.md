...
jobs:
  type-check:
    ...
    steps:
      ...
      - name: Install dev dependencies (pinned lockfile)
...
Dev dependencies are managed as a `requirements-dev.in` / `requirements-dev.lock` pair (rev 7):
- `deploy/energy-stack/requirements-dev.in` is human-edited intent: the unpinned set of test + type-checking packages (`pytest`, `pytest-asyncio`, `tzdata`, `mypy`, `pydantic`, `import-linter`, `types-requests`, plus any `types-*` stubs discovered during PR 1's `--install-types` review).
- `deploy/energy-stack/requirements-dev.lock` is generated: every direct AND transitive dependency pinned to an exact `==X.Y.Z`, captured from a clean Python 3.13 `pip install -r requirements-dev.in && pip freeze` run.
CI and local installs always read the `.lock`. Toolchain refreshes (`mypy` / `pydantic` / `import-linter` / `grimp` / `pathspec` / etc. upgrades) are deliberate "refresh typecheck toolchain" PRs that re-run the freeze and update both files in one commit. Pinning only the four named direct dependencies is insufficient — `import-linter` behavior is downstream of `grimp`, `mypy` behavior is downstream of `mypy_extensions` + `pathspec` + `typing_extensions`, and study-window stability requires the full tree to be locked. Service-specific runtime requirements are still loose-pinned per-service since CI only installs them so mypy can resolve symbol types from typed libraries.
...
├── requirements-dev.in                             # NEW (PR 1): human intent — pytest, mypy, pydantic, import-linter, types-*
├── requirements-dev.lock                           # NEW (PR 1): generated — every direct + transitive dep pinned ==X.Y.Z
...
- Add discovered stubs to `requirements-dev.in` and regenerate `requirements-dev.lock` (clean-venv pip install + pip freeze).
...
3. **Convert sibling imports to package-relative form.** Inside the service directory, convert cross-file imports from script-style to relative form. Patterns to handle:
   ...
   - Verify the conversion with two greps. Run BOTH before AND after conversion: pre-conversion finds what needs fixing; post-conversion should return zero results for the first grep (no remaining bare-name imports) and only properly-qualified references for the second grep:
     - `grep -rn "^from [a-z0-9_]* import\|^import [a-z0-9_]*$\|    from [a-z0-9_]* import" deploy/energy-stack/<service_underscored>/` — bare-name imports of project modules. The character class includes digits (`[a-z0-9_]`) because some modules contain numbers (e.g., `pjm_5cp.py` in the scheduler).
     - `git grep -nE "[\"']<service>\\.[a-z0-9_]*\\." deploy/energy-stack/<service_underscored>/` — string-form module references in monkeypatch / mocks. Same digit-inclusive character class.
...
5. **Update Dockerfile:**
   ...
   - CMD is `["python", "-m", "<service_underscored>.<entrypoint_module>"]`. Replaces the existing direct script invocation. The entrypoint module per service — verified against each service's existing Dockerfile CMD line:
     - `hvac_scheduler.app` (scheduler — file is `app.py`)
     - `hvac_scheduler_watchdog.check` (watchdog — file is `check.py`, NOT `app.py`)
     - `comed_poller.poller`, `eagle_poller.poller`, `refoss_poller.poller`, `thermostat_poller.poller` (pollers with `poller.py`)
     - `nws_poller.app`, `pjm_dm2_poller.app` (pollers using `app.py` — verified)
     - `ecowitt_ingest.app`, `haven_ingest.app` (ingest services use `app.py`)
     - `telegram_notifier.app` (uses `app.py` — verified)
     - **Always verify the actual entrypoint module name by inspecting the existing Dockerfile's CMD line before updating.** This list reflects state as of rev 5 verification; if a Dockerfile has been changed since, follow the Dockerfile not this list.
...
- **PR 1** — Infrastructure (mypy config, `run_typecheck.sh`, CI workflow, type-debt backlog, `requirements-dev.in` + `requirements-dev.lock` pair with all transitives pinned). First PR on the `feat/type-checker-pr1` branch.
...
- `deploy/energy-stack/requirements-dev.in` (human intent) + `deploy/energy-stack/requirements-dev.lock` (every direct AND transitive dependency pinned `==X.Y.Z`). The pair replaces the prior `requirements-dev.txt`. Versions selected by Task 2 of plan: install `requirements-dev.in` into a clean Python 3.13 venv, capture full `pip freeze` output, write that to `requirements-dev.lock`. CI and local installs both read `.lock`. Range specifiers are forbidden in `.lock` during the OSF-locked study window. mypy line: 2.x (2.0 GA 2026-05-06; default flips listed in §8.2). Toolchain refreshes are deliberate "refresh typecheck toolchain" PRs.
...
- **`types-*` discovery:** run `mypy --install-types --non-interactive --dry-run` against the installed deps. **Review each suggested stub package BEFORE adding to `requirements-dev.in` (and regenerating `requirements-dev.lock`):** only add stubs for libraries that do NOT already ship `py.typed` natively. If a stub package is suggested for a library we have `ignore_missing_imports = true` for, prefer removing the override (and using the stubs) over installing AND keeping the ignore — having both can cause stub/override conflicts. Document each kept/dropped stub in the type-debt backlog.
...
**Mitigation:** use a `requirements-dev.in` / `requirements-dev.lock` pair (rev 7). The `.lock` file pins EVERY direct and transitive dependency to an exact `==X.Y.Z` captured from a clean Python 3.13 `pip install -r requirements-dev.in && pip freeze`. CI and local installs read `requirements-dev.lock`. Toolchain refreshes are deliberate "refresh typecheck toolchain" PRs that re-run the freeze and update both files in one commit. Pinning only the four named direct dependencies is insufficient: `import-linter` behavior is downstream of `grimp`, `mypy` behavior is downstream of `mypy_extensions` + `pathspec` + `typing_extensions`, and study-window stability requires the full tree to be locked.

**mypy major version: 2.x.** mypy 2.0 was released 2026-05-06; the PR 1 freeze on 2026-05-21 captured `mypy==2.1.0` plus its transitive set. The rollout uses mypy 2.x, NOT 1.x — there is no reason to ship intentionally-older mypy for a study window that begins in 2026 when 2.x is the supported line. Notable mypy 2.0 default changes that may surface findings during PR 1 Task 7 bootstrap or per-service triage gates: `--local-partial-types` is now default-on; `--strict-bytes` is now default-on (PEP 688 — `bytearray`/`memoryview` no longer assignable to `bytes`); `--ignore-missing-imports` now applies consistently to all packages (legacy ≤ 0.812 special-case retired). The `pyproject.toml` mypy config (Task 3) explicitly sets these flags to their mypy 2 defaults so intent is documented rather than relying on default-drift between minor versions.
...
**Mitigation:** during per-service migration, run `mypy --install-types --non-interactive --dry-run` to suggest available stub packages. Add any relevant ones to `requirements-dev.in` and regenerate `requirements-dev.lock`. Document in the type-debt backlog.
...
| 2026-05-21 | draft (rev 7) | Seventh-pass review — Chris operator pre-execution review triggered by PR 1 Task 2 implementation surfacing two correctness issues. (R7A) **mypy major version: 1.x → 2.x.** mypy 2.0 was GA'd 2026-05-06, just 15 days before this PR 1 freeze. PR 1 Task 2's clean-Python-3.13 `pip install mypy` resolved to `mypy==2.1.0`, not the 1.x band that rev 6 still referenced. Rather than downpin to a deliberately-older 1.x for a study window starting in 2026, the rollout adopts mypy 2.x. Notable mypy 2.0 default changes that may surface findings during PR 1 Task 7 bootstrap or per-service triage gates: `--local-partial-types` now default-on; `--strict-bytes` now default-on (PEP 688 — `bytearray`/`memoryview` no longer assignable to `bytes`); `--ignore-missing-imports` now applies consistently to all packages (legacy ≤ 0.812 special-case retired). §8.2 risk-mitigation section documents the version + flag intent so the `pyproject.toml` mypy config (Task 3) explicitly sets these to mypy 2 defaults rather than relying on default-drift between minor versions. (R7B) **Dev-deps as `.in` / `.lock` pair.** Pinning only the four named direct dependencies (mypy + pydantic + import-linter + types-requests) is insufficient for study-window stability: `import-linter` behavior is downstream of `grimp`; `mypy` behavior is downstream of `mypy_extensions` + `pathspec` + `typing_extensions`. Replaced the rev 6 single `requirements-dev.txt` pin model with a `requirements-dev.in` (human intent, unpinned) + `requirements-dev.lock` (every direct AND transitive dep pinned `==X.Y.Z`, captured from clean-venv `pip install -r requirements-dev.in && pip freeze`) pair. CI and local installs both read `.lock`. Toolchain refreshes are deliberate "refresh typecheck toolchain" PRs that re-run the freeze and update both files in one commit. §4 CI sample, §4.4 file-structure diagram, §7 Phase 1 deliverables, §8.2 mitigation, plan Tech-Stack line, plan Task 2 (full rewrite), plan Task 5 CI install, plan Task 7 local install, plan Task 9 stub-discovery flow, plan Task 10 PR-body template all updated. Sources: [mypy 2.0 release blog](https://mypy-lang.blogspot.com/2026/05/mypy-20-relased.html), [Mypy 2.0 changelog](https://mypy.readthedocs.io/en/stable/changelog.html). |
