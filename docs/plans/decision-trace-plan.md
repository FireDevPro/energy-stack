...
- [arm_calendar.py](../../deploy/energy-stack/hvac-scheduler/arm_calendar.py) — 12-period A/B calendar already wired; trace rows align with its `arm` tag scheme when `SCHEDULER_MODE=experiment`.
...
| Cross-correlation fields on every trace line | `tick_id` (UUID4 per scheduler tick, JSON FIELD only — **NOT a Loki label**, would explode cardinality). When `SCHEDULER_MODE=experiment`, also tag with `arm` (read from `arm_calendar.current_arm_at(now_ct)`) and `scheduler_mode` so trace lines correlate with the existing arm-tagged `hvac.actions` / `hvac.decisions` rows. In `shadow` mode `arm` is omitted (no arm context pre-experiment). |
...
- New trace helper `_trace(event, level, verbose, **fields)` in `app.py` (one place; try/except wraps `log()`; honours `SCHEDULER_DECISION_TRACE_VERBOSE`; auto-inlines `tick_id`, `scheduler_mode`, and `arm` (when in `experiment` mode) into every emitted line).
...
- New log emission at the caller side in `app.py:run_per_tick_overlays`, immediately after `evaluate_price_overlay` returns. `price_overlay.py` stays pure (zero `log()` calls — the module already has that property, worth preserving). Emit one line per call with:
  ...
  - `scheduler_mode` (one of `shadow` / `experiment` / `production`)
  - `arm` (only when `scheduler_mode=experiment`; from `arm_calendar.current_arm_at`)
...
- `test_arm_field_present_only_in_experiment_mode` — drive `run_per_tick_overlays` with `SCHEDULER_MODE=shadow` and with `=experiment`; assert `arm` absent in the first, present in the second.
...
- Fields: `tick_id`, `scheduler_mode`, `arm` (when applicable), `schedule_cool_f`, `price_overlay_tier`, `price_cool_f`, `fivecp_active` (bool), `fivecp_scopes_fired` (list), `fivecp_cool_f`, `effective_cool_f`, `winning_layer` ∈ `{schedule, price_overlay, 5cp}`, `reason_code`.
...
- New trace at every `safety_supervisor.evaluate(...)` call site. Fields: `tick_id`, `scheduler_mode`, `arm` (when applicable), `proposed_cool_f`, `proposed_heat_f`, `indoor_temp_f`, `indoor_temp_available` (bool), `decision` ∈ `{approved, clamped, emergency}`, `reason_code`, `final_cool_f`, `final_heat_f`.
...
- Fields: `tick_id`, `scheduler_mode`, `arm` (when applicable), `decision_for_date`, `day_type`, `selected` (bool), `hour_ct`, `depth_f`, `reason_code`.
...
- New trace at the call site in `app.py:run_decision` and `app.py:run_decision_revisit`. Fields: `tick_id`, `scheduler_mode`, `arm` (when applicable), `decision_for_date`, `winning_day_type`, `evaluation_tape` (read from `reasons["evaluation_tape"]`, inlined into the log line). Level `info`.
...
| `arm` field on trace lines collides with the existing `hvac.actions` / `hvac.decisions` `arm` tag scheme | Use the same value semantics as `arm_calendar.current_arm_at` (literal `"A"` / `"B"` / absent). Phase 1 acceptance test covers presence-vs-mode. |
