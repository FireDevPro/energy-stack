...
| B | RTP/DTOD live price-responsive HVAC controller with weather/day-type scheduling, price-aware pre-cool, 5CP capacity-risk planning/telemetry, and safety supervision. Pushes setpoints to CTK04AE. | Scheduler commands → CTK04AE. | If scheduler dies → CTK04AE program resumes (= effectively-A behavior during that window). Per Section 5: hour classified as B-down, EXCLUDED from primary. |

**Live setpoint authority** is the RTP/DTOD price overlay (§2), the day-type schedule baseline, and the safety supervisor's [65, 86]°F clamp. 5CP capacity-risk signals (§3) MAY inform pre-cool/day-type planning decisions at the daily 21:00 decision when sufficient current-season official metered-load history exists, and ARE reported as process telemetry. **5CP risk does NOT independently force live setpoint changes.** A bare `fivecp_active=True` with normal prices does not move the effective cool setpoint. Severe price events still drive shutoff: ≥10¢/kWh adds +3°F (elevated tier); ≥20¢/kWh overrides to 85°F (scarcity tier).
...
- The controller's capacity-risk planning/telemetry is inactive by design.

**Feed-health rule for PJM/5CP inputs** (revised — 5CP demoted from live-control authority per §3): price (`comed.prices`) and weather/forecast (`nws.forecast`) remain required live-control inputs at all times during Arm B. PJM/5CP inputs (`pjm.metered_load`, `pjm.load_forecast`, `pjm.inst_load`) are required ONLY when making 5CP-informed planning decisions (the §7 single-day forecast 5CP-risk path at the 21:00 daily decision). Missing or insufficient PJM/5CP data does NOT down-classify an Arm B hour to B-fallback — live control no longer depends on them. When 5CP data is insufficient for the planning decision (< 168 distinct current-season official hourly observations for the relevant scope), the day-type decision falls back to the weather/price logic and the reasoning records `insufficient_current_season_history`. No prior-year fallback is used for control or planning decisions.
...
14. **5CP live-authority demotion + season-5th baseline fix**: pre-OSF correction. 5CP capacity-risk signals are demoted to planning/telemetry only; they do NOT independently force live setpoint changes (`fivecp_active=True` no longer raises `effective_cool_f`). `winning_layer` in `decision_trace.layer_resolution` remains actual-control only and never becomes `"5cp"` after this change. Mid-period re-push triggers on price-tier transitions and supervisor behavior; a bare 5CP active/inactive transition does not push. `update_season_5th_highest()` returns `float | None` — None means insufficient current-season official metered-load history (< 168 distinct hourly observations). Prior-year fallback is removed from control/planning paths. Day-type decision at 21:00 falls back to weather/price logic when the baseline is None and records `insufficient_current_season_history` in reasoning. Existing `hvac.5cp_state` and `fivecp_eval` telemetry are preserved so post-hoc analysis can reconstruct when 5CP would have fired.
...
- ~~Capacity-risk operating window precise dates (§5.1): currently locked as 2026-06-01 → 2026-09-30. Confirm against PJM historical 5CP-distribution data before OSF freeze. PJM 5CP hours over recent summers (2020-2024) almost exclusively fall June-August; September is borderline. Audit data informs whether end-of-window should be 09-15, 09-30, or 10-15.~~ **Resolved 2026-05-18:** 09-30 is the regulatory 5CP eligibility window endpoint per PJM Manual 19 and ComEd Attachment M-2 (already cited in `docs/EXPERIMENT_DESIGN.md` line 569 and `deploy/energy-stack/hvac_scheduler/pjm_5cp.py:140`). Not an empirical choice — fixed by tariff. The empirical confirm question was framed wrong in the original §15 wording: the audit-grade question is "does PJM Manual 19's definition match what we wrote?" (yes, 09-30), not "what date does the data suggest?" Three years of ComEd RTP price data (2023-2025) independently confirm zero ≥10¢/kWh stress events in October at this load center, which is consistent with the regulatory boundary but not the source of truth for it. §5.1 updated with the regulatory citation co-located with the date.
- Broader repo/docs audit for "PJM/5CP feed" and "5CP prediction" wording: review `docs/HVAC_LOGIC.md`, code comments, and `deploy/energy-stack/hvac_scheduler/pjm_5cp.py` for instances where the distinction between (a) live capacity-risk detector and (b) official post-season 5CP list matters. Replace only where misleading. Do NOT rename historical artifacts like `pjm.coincident_peak` InfluxDB measurement or the `pjm_5cp.py` module filename — those remain valid as post-season truth-source terminology.
