...
2. **Ecowitt canonical weather fields.** Are `ch1_*` or `outdoor_*` canonical for weather-vector analysis? Evidence: spec `:209` and `tools/analysis/weather_vector.py:97` use `ch1_*`; `docs/SERVICES.md:78` and `deploy/energy-stack/ecowitt_ingest/app.py:48` call `outdoor_*` canonical.
