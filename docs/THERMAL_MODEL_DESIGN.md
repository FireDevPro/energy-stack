...
The literature consensus (Bacher & Madsen 2011, Chen et al. 2020 Applied Energy, ORNL thermal-mass studies, NREL ResStock + Alfa work, DOE GEB Roadmap 2021) is that house-specific parameters identified from **operational data** (not blueprints, not Manual J, not EnergyPlus calibration) capture 20–57% of the achievable optimization gain over a fixed-rule scheduler. The cited methodology in every case is some flavor of RC-network parameter identification fed by indoor temperature, outdoor temperature, equipment state, and solar irradiance.
...
- Chen, Y., Tong, Z., Zheng, Y., Samuelson, H., & Norford, L. (2020). *Transfer learning with deep neural networks for model predictive control of HVAC and natural ventilation in smart buildings*. Applied Energy, 271, 115147. Documents 20–57% optimization gain over fixed-rule schedulers when house parameters are identified from operational data.
