...
**Data lineage** (per PJM DM2 OpenAPI spec): `current_zone_load_mw` reads PJM's `inst_load` feed (area=COMED, ~5-min cadence, "approximate, NOT official PJM Loads" — the real-time directional signal). `season_to_date_5th_highest_mw` reads PJM's `hrl_load_metered` feed (zone=CE, official metered values, daily publish with up to 90-day correction window). Two feeds, two purposes; both required.
