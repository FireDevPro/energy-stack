...
All co-primary outcomes are reported as the matched-pair median difference (Arm B minus Arm A) with bootstrap 95% CI. The dollar-valued matched-pair outcomes — **O1 (weekly HVAC actual cost) and O4 (weekly whole-home actual cost)** — are reported as an absolute dollar delta side-by-side with `percent_of_arm_a` (= matched-pair median of `(Δ / arm_a_value) × 100`) for reader intuition. Percent is never reported as the sole headline — the absolute dollar figure is the trustworthy primary; the percent provides context. **O2 (capacity-charge avoidance) reports the absolute dollar delta only**; its bootstrap is taken across PJM 5CP hours under each arm rather than across matched weekly pairs, so the "Arm A matched cost" denominator used for O1 and O4 does not transfer. A percent framing for O2 can be added in a future amendment if a clean denominator is pre-committed.
...
**O6 — All-day HVAC load-shift / rebound diagnostic.** Diagnostic measures of how Arm B redistributes HVAC load across price tiers, 5CP-active windows, and post-event rebound periods. Not a single number; reported as a panel:
...
**2. CT-485 / ComfortNet HVAC-state.** ComfortNet/CT-485 telemetry is a data source, not a control authority. It is used for HVAC-state observation (compressor / blower / staging), Refoss Tier 3 imputation per Rule 1, and comfort/system-state reconstruction for diagnostics. It does not control the thermostat, does not decide Arm A / Arm B behavior, and is not on any controller authority path. Missing intervals are not imputed. ComfortNet downtime does not by itself invalidate any outcome; a specific outcome subcomponent invalidates only if its computation actually depends on ComfortNet-derived state (e.g., Rule 1 Tier 3 imputation for a given gap; certain O6 sub-panels if explicitly defined to require compressor/blower state). Per-week ComfortNet coverage is reported as descriptive context.
...
For each matched pair, the per-pair table reports the six Stage 4 weather matching-vector components (CDD, mean outdoor enthalpy, total solar irradiance, mean wind speed, max temperature, max dewpoint) so readers can see how the pair was matched, plus the indoor temperature distribution (median, 90th, 95th percentile during the 13:00-20:00 CT 5CP-eligibility window — per §3 Appendix A — for both arms) and the per-outcome differences. CDD appears as one component of the matching vector for matching-transparency purposes; it is not used as a standalone reader-facing diagnostic, outcome denominator, or eligibility metric anywhere in the analysis.
...
1. **Headline:** matched-pair median Arm B − Arm A difference with bootstrap 95% CI for each co-primary outcome (O1 weekly HVAC actual cost, O4 weekly whole-home actual cost, O2 Layer 1 capacity-charge $ delta). O1 and O4 are reported as an absolute dollar delta side-by-side with `percent_of_arm_a` for reader intuition; the absolute dollar figure is the trustworthy primary, and percent is never the sole headline (denominator matters). O2 reports the absolute dollar delta only (its bootstrap is over PJM 5CP hours, not matched weekly pairs, so the O1/O4 percent denominator does not transfer; a percent framing for O2 can be added in a future amendment if a clean denominator is pre-committed). O2 Layers 2 and 3 reported as descriptive side-tables. Secondary outcomes (O3 peak kW, O7 HVAC kWh, O8 whole-home kWh) reported alongside.
...
7. The statistical analysis plan in §7: the 6-component weather summary vector, the Hungarian matching algorithm minimizing total Mahalanobis distance (with Σ from the locked ERA5 baseline covariance), the T=2.5 Mahalanobis quality threshold, the 10,000-resample stationary bootstrap with percentile 95% CI as the primary inference, the SCED sign-flip randomization test as the secondary inference, and the pre-committed sensitivity analyses. Headline reporting (per §8) is the absolute matched-pair median $ delta with bootstrap 95% CI. For O1 and O4 only, `percent_of_arm_a` is reported side-by-side as reader intuition; percent is never the sole headline. O2 reports absolute $ delta only.
