...
PJM's final-published 5CP hour lists for the RTO (5 PJM Five Peak hours) and the ComEd zone (5 ComEd Five Peak hours), released mid-October after the season closes per [PJM Manual 19 §4.3](https://www.pjm.com/-/media/documents/manuals/m19.ashx). The Arm B 5CP detector's hour-by-hour `fivecp_active` accuracy (what it WOULD have fired under the pre-§11-#14 live-authority design, recorded in `hvac.5cp_state` / `fivecp_eval` telemetry) is reported descriptively as a process metric (see Detector accuracy report below); it does NOT define O2 and does not drive realized shutoff windows. O2 is computed at the PJM-published peak hours regardless of which hours the detector flagged.
...
Separately, the Arm B 5CP detector's hour-by-hour `fivecp_active` state during summer Y (recorded in `hvac.5cp_state` and `fivecp_eval` telemetry) is cross-referenced against PJM's October-published 5CP hour list. **Important framing (binding spec §11 #14):** 5CP was demoted from live setpoint authority to planning/telemetry only; the detector's `fivecp_active=True` state during summer Y reflects what 5CP WOULD have fired under the pre-§11-#14 live-authority design, NOT actual held-shutoff windows during the experiment. The cross-reference therefore characterizes the detector ALGORITHM (would-have-fired hours vs published 5CP hours) as an engineering subsystem, not realized shutoff behavior. Reported: would-fire-true-positive rate (detector active during a published 5CP hour), would-fire-false-positive rate (detector active during a non-5CP hour), would-fire-false-negative rate (detector inactive during a published 5CP hour). This is decoupled from O2's outcome statement; actual Arm B shutoff windows are driven by the price-overlay scarcity tier and are visible in `hvac.actions`.
